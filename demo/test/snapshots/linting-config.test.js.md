...
> Snapshot 1
    ...
    ␊
    demo/test/published-config.test.js␊
      75:42  warning  Replace \`.files.map((file)·=>·file.path)\` with \`⏎····.files.map((file)·=>·file.path)⏎····\`  prettier/prettier␊
    ␊
    demo/test/snapshots/rule-matrix.js␊
       56:3  error    iterators/generators require regenerator-runtime, which is too heavyweight for this guide to allow them. Separately, loops should be avoided in favor of array iterations  no-restricted-syntax␊
      105:9  warning  Each then() should return a value or throw                                                                                                                                 promise/always-return␊
    ␊
    test/example.js␊
